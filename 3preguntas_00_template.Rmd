...
pharmacoData$cellLine -> cell_lines
unique(cell_lines) -> Ucell_lines
length(Ucell_lines)
...
```{r Answer2}
length(tapply(pharmacoData$concentration, pharmacoData$study, function(x){unique(x)}))
```
...
```{r Answer3}
Viability <- pharmacoData$viability
hist(Viability)
